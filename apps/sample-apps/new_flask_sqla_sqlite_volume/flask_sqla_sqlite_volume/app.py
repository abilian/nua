...
@app.route("/<int:student_idt>/")
def student(student_idt):
    student = Student.query.get_or_404(student_idt)
...
@app.route("/<int:student_idt>/edit/", methods=("GET", "POST"))
def edit(student_idt):
    student = Student.query.get_or_404(student_idt)
    if request.method == "POST":
        firstname = request.form["firstname"] or "<unknown>"
        lastname = request.form["lastname"] or ""
...
@app.post("/<int:student_idt>/delete/")
def delete(student_idt):
    student = Student.query.get_or_404(student_idt)
